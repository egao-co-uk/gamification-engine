...
def main(argv=sys.argv):
    ...
    with transaction.manager:
        ...
        for task in tasks:
            if task["task_name"] in enginetasks.keys():
                ...
                sess.execute(
                    m.t_taskexecutions.update()
                    .values({"locked_at": dt_now()})
                    .where(m.t_taskexecutions.c.id == task["execution_id"])
                )
    ...
    for task in to_run:
        with transaction.manager:
            ...
            log.info(
                "Running task %(task_name)s (ExecutionID: %(execution_id)s)"
                % {"task_name": task["task_name"], "execution_id": task["execution_id"]}
            )
            ...
            sess.execute(
                m.t_taskexecutions.update()
                .values({"finished_at": dt_now(), "log": logged, "success": success})
                .where(m.t_taskexecutions.c.id == task["execution_id"])
            )
