...
def calc_distance(latlong1, latlong2):
    ...
    return func.sqrt(
        func.pow(111.2 * (latlong1[0] - latlong2[0]), 2)
...
def datetime_trunc(field, timezone):
    return (
        "date_trunc('%(field)s', CAST(to_char(NOW() AT TIME ZONE %(timezone)s, 'YYYY-MM-DD HH24:MI:SS') AS TIMESTAMP)) AT TIME ZONE %(timezone)s"
        % {"field": field, "timezone": timezone}
    )
