...
r_status = JSchema(schema={"status": sw.property(sw.Types.string, nullable=False)})
...
r_subjectlist = JSchema(
    schema={"subjects": sw.array_property(items=sw.object_property(properties=r_subject.get_json_schema()))}
)

b_subjectlist = JSchema(
    schema={
...
b_subject_id = JSchema(schema={"subject_id": sw.property(sw.Types.number, nullable=True)})


r_subjecttype = JSchema(
    schema={
        "id": sw.property(sw.Types.string, nullable=False),
...
r_subjecttypelist = JSchema(
    schema={"subjecttypes": sw.array_property(items=sw.object_property(properties=r_subjecttype.get_json_schema()))}
)
...
r_variablelist = JSchema(
    schema={"variables": sw.array_property(items=sw.object_property(properties=r_variable.get_json_schema()))}
)
...
r_timezonelist = JSchema(
    schema={"timezones": sw.array_property(items=sw.object_property(properties=r_timezone.get_json_schema()))}
)

# This currently only accepts the parameters which are needed for the leaderboard_creation form
# To be extended if needed
b_createachievement = JSchema(
    schema={
        "name": sw.property(sw.Types.string, nullable=False),
        ...
        "domain_subject_ids": sw.array_property(items=sw.property(sw.Types.number, nullable=False), nullable=True),
