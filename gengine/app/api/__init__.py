...
from gengine.app.permissions import (
    perm_global_search_subjects,
    perm_global_manage_subjects,
    perm_global_search_subjecttypes,
    perm_global_list_variables,
    perm_global_list_timezones,
    perm_global_manage_achievements,
)
from gengine.base.model import update_connection
from sqlalchemy.sql.sqltypes import Integer, String

from gengine.app.api.resources import (
    SubjectCollectionResource,
    SubjectResource,
    SubjectTypeCollectionResource,
    VariableCollectionResource,
    ApiResource,
)
from gengine.app.api.schemas import (
    r_status,
    r_subjectlist,
    b_subjectlist,
    b_subject_id,
...
@api_route(
    ...
    request_method="POST",
    name="list",
    context=SubjectCollectionResource,
    renderer="json",
    api=sw.api(
        tag="subjects",
...
def subjects_search_list(request, *args, **kw):
    ...
    if parent_subjecttype_id is not None:
        q = q.where(t_subjecttypes_subjecttypes.c.part_of_id == parent_subjecttype_id)
    ...
    for r in result:
        if not r["id"] in subjects:
            ...
            inherited_by_name = (
                r["inherited_by_name"] if "inherited_by_name" in r and r["inherited_by_name"] is not None else ""
            )
            inherited_by_subjecttype_id = (
                r["inherited_by_subjecttype_id"]
                if "inherited_by_subjecttype_id" in r and r["inherited_by_subjecttype_id"] is not None
                else ""
            )
            subjects[r["id"]] = {
                "id": r["id"],
    ...
    return r_subjectlist.output({"subjects": list(subjects.values())})


@api_route(
    path="/subjects/{parent_id}",
    request_method="POST",
    name="add_subject",
    context=SubjectResource,
    renderer="json",
    api=sw.api(
        tag="subjects",
...
def subject_add_to_parent(request, *args, **kw):
    ...
    q = t_subjects_subjects.select().where(
        and_(
    ...
    return r_status.output({"status": "ok"})
...
def subject_remove_from_parent(request, *args, **kw):
    ...
    if r:
        q = t_subjects_subjects.delete().where(
            and_(
                t_subjects_subjects.c.subject_id == request.validated_params.body["subject_id"],
                t_subjects_subjects.c.part_of_id == parent_row["id"],
            )
...
@api_route(
    ...
    request_method="GET",
    name="list",
    context=SubjectTypeCollectionResource,
    renderer="json",
    api=sw.api(
        tag="subjecttypes",
        ...
        responses={
            200: sw.response(schema=r_subjectlist.get_json_schema()),
            400: sw.response(
                schema=r_status.get_json_schema(),
                description="""
        """,
            ),
        },
...
@api_route(
    ...
    request_method="GET",
    name="list",
    context=VariableCollectionResource,
    renderer="json",
    api=sw.api(
        tag="variables",
