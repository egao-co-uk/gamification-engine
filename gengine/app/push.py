...
def send_push_message(
    ...
    message_count = DBSession.execute(
        select([func.count("*").label("c")], from_obj=t_subject_messages).where(
            and_(
    ...
    rows = DBSession.execute(
        select(
            [t_subject_device.c.push_id, t_subject_device.c.device_os],
            from_obj=t_subject_device,
        )
        .distinct()
        .where(t_subject_device.c.subject_id == subject_id)
    ).fetchall()
