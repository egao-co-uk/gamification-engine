...
def init_admin(
    ...
    class ModelViewProgress(ModelView):
        ...
        column_filters = (IntEqualFilter(Subject.id, "SubjectID"), Achievement.id)
    ...
    class PermissionInlineModelForm(InlineFormAdmin):
        ...
        form_choices = {"name": sorted(list(yield_all_perms()), key=lambda x: x[1])}
    ...
    class ModelViewTask(ModelView):
        ...
        form_choices = {"task_name": [(x, x) for x in enginetasks.keys()]}
        ...
        @action(
            "execute",
            "Execute now",
            "Are you sure you want to execute the selected task?",
        )
        def action_execute(self, ids):
            ...
            tasks = DBSession.execute(t_tasks.select().where(t_tasks.c.id.in_(*ids))).fetchall()

            for task in tasks:
                result = get_task_registry().execute(name=task["task_name"], config=task["config"])
    ...
    if not override_flaskadminapp:
        ...
        adminapp.config.update(dict(PREFERRED_URL_SCHEME="https"))
    else:
    ...
    def inject_version():
        return {
            "gamification_engine_version": pkg_resources.get_distribution("gamification-engine").version,
    ...
    admin.add_view(
        ModelView(
            ...
            DBSession,
            category="Rules",
            name="Achievement Property Values",
        )
    )
    admin.add_view(
        ModelView(
            ...
            DBSession,
            category="Rules",
            name="Achievement Reward Values",
        )
    )
    ...
    admin.add_view(
        LeaderboardCreationView(
            name="Create Leaderboards",
            endpoint="leaderboard_creation",
            category="Rules",
        )
    )
