...
from sqlalchemy import Table, ForeignKey, func, select, and_, or_, text, Column, event
...
from gengine.app.cache import (
    cache_general,
    cache_achievements_subjects_levels,
    cache_achievements_by_subject_for_today,
    cache_translations,
)
from gengine.app.formular import (
    evaluate_condition,
...
t_subjecttypes_subjecttypes = Table(
    "subjecttypes_subjecttypes",
    Base.metadata,
    Column("id", ty.Integer, primary_key=True),
    Column(
        "subjecttype_id",
        ty.Integer,
        ForeignKey("subjecttypes.id", ondelete="CASCADE"),
        index=True,
        nullable=False,
    ),
    Column(
...
""")
event.listen(
    t_subjecttypes_subjecttypes,
    "after_create",
    t_subjecttypes_subjecttypes_ddl.execute_if(dialect="postgresql"),
)
...
event.listen(
    t_subjects_subjects,
    "after_create",
    t_subjects_subjects_ddl.execute_if(dialect="postgresql"),
)
...
t_auth_tokens = Table(
    ...
    Column(
        ...
        TIMESTAMP(timezone=True),
        nullable=False,
        default=get_default_token_valid_time,
    ),
    Column(
        "created_at",
        TIMESTAMP(timezone=True),
...
t_auth_roles_permissions = Table(
    ...
    Column(
        "auth_role_id",
        ty.Integer,
        ForeignKey("auth_roles.id", use_alter=True, ondelete="CASCADE"),
        nullable=False,
        index=True,
    ),
...
t_achievements = Table(
    ...
    Column(
        ...
        ty.Integer,
        ForeignKey("achievementcategories.id", ondelete="SET NULL"),
        index=True,
        nullable=True,
    ),
    ...
    Column(
        "comparison_type",
        ty.Enum("global", "context_subject", "relations", "none", name="comparison_types"),
        default="none",
    ),
    # This one is the actual player, that will "win" the achievement
    Column(
        "player_subjecttype_id",
        ty.Integer(),
        ForeignKey("subjecttypes.id", ondelete="CASCADE"),
        nullable=False,
        index=True,
    ...
    Column(
        "context_subjecttype_id",
        ty.Integer(),
        ForeignKey("subjecttypes.id", ondelete="RESTRICT"),
        nullable=True,
        index=True,
    ),
    ...
    Column(
        "view_permission",
        ty.Enum("everyone", "own", name="achievement_view_permission"),
        default="everyone",
    ),
    ...
    Column(
        "maxmin",
        ty.Enum("max", "min", name="goal_maxmin"),
        nullable=True,
        default="max",
    ),
...
t_achievement_compared_subjecttypes = Table(
    "achievement_compared_subjects",
    Base.metadata,
    Column("id", ty.Integer, primary_key=True),
    Column(
        "achievement_id",
...
t_achievement_domain_subjects = Table(
    "achievement_domain_subjects",
    Base.metadata,
    Column("id", ty.Integer, primary_key=True),
    Column(
        "achievement_id",
...
t_evaluations = Table(
    ...
    # For which period?
    Column(
        "achievement_date", TIMESTAMP(timezone=True), nullable=True, index=True
    ),  # To identify the goals for monthly, weekly, ... achievements;
...
Index(
    "idx_evaluations_date_not_null_unique",
...
Index(
    "idx_evaluations_date_null_unique",
    t_evaluations.c.subject_id,
    t_evaluations.c.achievement_id,
    t_evaluations.c.level,
    unique=True,
...
Index(
    "idx_progress_date_not_null_unique",
...
t_variables = Table(
    ...
    Column(
        "increase_permission",
        ty.Enum("own", "admin", name="variable_increase_permission"),
        default="admin",
    ),
    Column(
        "created_at",
...
t_achievement_trigger_steps = Table(
    "achievement_trigger_steps",
    Base.metadata,
    Column("id", ty.Integer, primary_key=True),
    Column(
        "achievement_trigger_id",
        ty.Integer,
        ForeignKey("achievement_triggers.id", ondelete="CASCADE"),
        nullable=False,
        index=True,
    ),
    ...
    Column(
        "condition_type",
        ty.Enum("percentage", name="achievemennt_trigger_condition_types"),
        default="percentage",
    ),
    ...
    Column(
        "action_type",
        ty.Enum("subject_message", "increase_value", name="achievement_trigger_action_types"),
        default="subject_message",
    ),
    Column(
        "action_translation_id",
        ty.Integer,
        ForeignKey("translationvariables.id", ondelete="RESTRICT"),
        nullable=True,
    ),
...
t_achievement_trigger_step_executions = Table(
    "achievement_trigger_executions",
    Base.metadata,
    Column("id", ty.BigInteger, primary_key=True),
    # Which step?
    Column(
        "trigger_step_id",
        ty.Integer,
        ForeignKey("achievement_trigger_steps.id", ondelete="CASCADE"),
        nullable=False,
    ),
    # For whom?
    Column(
    ...
    Index(
        "ix_achievement_trigger_executions_combined",
        "trigger_step_id",
        "subject_id",
        "execution_level",
        "achievement_date",
...
t_achievements_achievementproperties = Table(
    ...
    Column(
        "property_id",
        ty.Integer,
        ForeignKey("achievementproperties.id", ondelete="CASCADE"),
        index=True,
        nullable=False,
    ),
    # Can be a formula...
...
t_achievements_rewards = Table(
    ...
    Column(
        "value_translation_id",
        ty.Integer,
        ForeignKey("translationvariables.id"),
        nullable=True,
    ),
    # Valid from which level (higher level overrides entries for lower levels)
...
t_translations = Table(
    "translations",
    Base.metadata,
    Column("id", ty.Integer, primary_key=True),
    Column(
        "translationvariable_id",
        ty.Integer,
        ForeignKey("translationvariables.id", ondelete="CASCADE"),
        nullable=False,
    ),
    Column(
        "language_id",
...
class AuthUser(ABase):
    ...
    def get_or_create_token(self):
        ...
        if not tokenObj:
            ...
            tokenObj = AuthToken(auth_user_id=self.id, token=token)
    ...
    def may_increase(cls, variable_row, request, subject_id):
        ...
        if request.has_perm(perm_global_increase_value):
            ...
            return True
        if (
            variable_row["increase_permission"] == "own"
            and request.subject
            and str(request.subject.id) == str(subject_id)
        ):
...
class SubjectDevice(ABase):
    ...
    def add_or_update_device(cls, subject_id, device_id, push_id, device_os, app_version):
        ...
        if device and (
            device["push_id"] != push_id or device["device_os"] != device_os or device["app_version"] != app_version
        ):
            uSession = update_connection()
            q = (
                t_subject_device.update()
                .values(
                    {
                        "push_id": push_id,
                        "device_os": device_os,
...
class Subject(ABase):
    ...
    def get_subjects(cls, subject_ids):
        return {
            x["id"]: x
            for x in DBSession.execute(t_subjects.select().where(t_subjects.c.id.in_(subject_ids))).fetchall()
        }

    @classmethod
    def set_relations(cls, subject_id, relation_ids):
        new_friends_set = set(relation_ids)
        existing_subjects_set = {
            x["id"]
            for x in DBSession.execute(
                select([t_subjects.c.id]).where(
                    t_subjects.c.id.in_(
                        [
                            subject_id,
                        ]
        ...
        existing_friends = {
            x["to_id"]
            for x in DBSession.execute(
                select([t_subjectrelations.c.to_id]).where(t_subjectrelations.c.from_id == subject_id)
            ).fetchall()
        }
        not_existing_friends = (
            new_friends_set
            - existing_subjects_set
            - {
                subject_id,
        ...
        if len(friends_to_delete) > 0:
            update_connection().execute(
                t_subjectrelations.delete().where(
                    and_(
                        t_subjectrelations.c.from_id == subject_id,
        ...
        if len(friends_to_append) > 0:
            update_connection().execute(
                t_subjectrelations.insert(),
                [{"from_id": subject_id, "to_id": f} for f in friends_to_append],
            )
    ...
    def full_output(cls, subject_id):
        ...
        friends = DBSession.execute(
            t_subjects.select(from_obj=j).where(t_subjectrelations.c.from_id == subject_id)
        ).fetchall()

        j = t_subjects.join(t_subjects_subjects, t_subjects_subjects.c.part_of_id == t_subjects.c.id)
        part_of_subjects = DBSession.execute(
            t_subjects.select(from_obj=j).where(t_subjects_subjects.c.subject_id == subject_id)
        ).fetchall()

        language = get_settings().get("fallback_language", "en")
        j = t_subjects.join(t_languages)
        subject_language = DBSession.execute(
            select([t_languages.c.name], from_obj=j).where(t_subjects.c.id == subject_id)
        ).fetchone()
        if subject_language:
        ...
        if get_settings().get("enable_user_authentication"):
            auth_user = DBSession.execute(
                t_auth_users.select().where(t_auth_users.c.subject_id == subject_id)
            ).fetchone()
            if auth_user:
                ret.update({"email": auth_user["email"]})
    ...
    def get_ancestor_subjects(cls, subject_id, of_type_id, from_date, to_date, whole_time_required):
        ...
        if whole_time_required:
            ...
            datestr = (
                "((%(ss)s.joined_at<=:from_date AND (%(ss)s.left_at IS NULL OR %(ss)s.left_at >= :from_date))"
        ...
        sq = (
            ...
            WITH RECURSIVE nodes_cte(subject_id, name, part_of_id, depth, path) AS (
                ...
                WHERE ss.subject_id = :subject_id AND """
                + (datestr % {"ss": "ss"})
                + """
            UNION ALL
        ...
        """
            ...
            .bindparams(subject_id=subject_id, from_date=from_date, to_date=to_date)
            .columns(
                subject_id=Integer,
                name=String,
                part_of_id=Integer,
    ...
    def get_descendent_subjects(cls, subject_id, of_type_id, from_date, to_date, whole_time_required):
        ...
        sq = (
            ...
            WITH RECURSIVE nodes_cte(subject_id, name, part_of_id, depth, path) AS (
                ...
                WHERE ss.part_of_id = :subject_id AND """
                + (datestr % {"ss": "ss"})
                + """
            UNION ALL
                ...
                WHERE c.part_of_id = p.subject_id AND """
                + (datestr % {"ss": "c"})
                + """
            ) SELECT * FROM nodes_cte
        """
    ...
    @classmethod
    def join_subject(cls, subject_id, part_of_id, join_date):
        q = t_subjects_subjects.insert().values(
            {"subject_id": subject_id, "part_of_id": part_of_id, "joined_at": join_date}
        )

        update_connection().execute(q)

    @classmethod
    def leave_subject(cls, subject_id, part_of_id, leave_date):
        q = (
            t_subjects_subjects.update()
            .values({"left_at": leave_date})
            .where(
                and_(
...
class Variable(ABase):
    ...
    def map_variables_to_rules(cls):
        ...
        q = select(
            [
                t_achievements.c.id.label("achievement_id"),
                t_variables.c.id.label("variable_id"),
            ]
        ).where(
            or_(
...
class Value(ABase):
    ...
    def increase_value(
        ...
        for sid in sid_set:
            condition = and_(
                t_values.c.datetime == at_datetime,
            ...
            current_value = DBSession.execute(
                select(
                    [
                        t_values.c.value,
                    ]
        ...
        for sid in sid_set:
            for achievement in achievements:
                ...
                compared_subjects = [
                    s for s in subjects[sid].values() if s["subjecttype_id"] in achievement["compared_subjecttypes"]
                ]
                csids = set(
                    [x["subject_id"] for x in compared_subjects]
                    + [
                        subject_id,
                    ]
                )
                q = t_progress.delete().where(
                    and_(
...
class AchievementCategory(ABase):
    ...
    def get_achievementcategory(cls, achievementcategory_id):
        return DBSession.execute(
            t_achievementcategories.select().where(t_achievementcategories.c.id == achievementcategory_id)
        ).fetchone()
...
class AchievementDate:
    ...
    def compute(cls, evaluation_timezone, evaluation_type, evaluation_shift, context_datetime):
        ...
        if evaluation_type == "yearly":
            if evaluation_shift:
                from_date = tzobj.localize(
                    (from_date.replace(tzinfo=None) - datetime.timedelta(seconds=evaluation_shift))
                )
        ...
        return AchievementDate(from_date=from_date.astimezone(tzobj), to_date=to_date.astimezone(tzobj))
...
class Achievement(ABase):
    ...
    def get_achievement(cls, achievement_id):
        achievement = rowproxy2dict(
            DBSession.execute(t_achievements.select().where(t_achievements.c.id == achievement_id)).fetchone()
        )
        compared_subjecttypes = [
            x["id"]
            for x in DBSession.execute(
                t_achievement_compared_subjecttypes.select().where(
                    t_achievement_compared_subjecttypes.c.achievement_id == achievement_id
                )
            ).fetchall()
        ]
        domain_subjects = [
    ...
    def get_achievements_by_subject_for_today(cls, subject):
        ...
        return cache_achievements_by_subject_for_today.get_or_create(
            key,
            generate_achievements_by_subject_for_today,
    ...
    def get_achievements_by_location(cls, latlng):
        ...
        q = select([t_achievements, distance]).where(
            or_(
                and_(t_achievements.c.lat == None, t_achievements.c.lng == None),
    ...
    def get_achievements_by_date(cls, date):
        ...
        q = t_achievements.select().where(
            and_(
                or_(
                    t_achievements.c.valid_start == None,
                    t_achievements.c.valid_start <= date,
                ),
                or_(
                    t_achievements.c.valid_end == None,
    ...
    @classmethod
    def get_relevant_subjects_by_achievement_and_subject(
        cls, achievement, subject, context_subject_id, from_date, to_date
    ):
        """
        ...
        from gengine.app.leaderboard import (
            RelationsLeaderBoardSubjectSet,
            GlobalLeaderBoardSubjectSet,
        ...
        def generate():
            q = select(
                ...
                and_(
                    ...
                    t_evaluations.c.achieved == True,
                ),
            ).order_by(t_evaluations.c.level.desc())
            return [x for x in DBSession.execute(q).fetchall()]

        return cache_achievements_subjects_levels.get_or_create(
            "%s_%s_%s_%s"
            % (
                str(subject_id),
                str(achievement_id),
                str(achievement_date),
                str(context_subject_id),
            ),
            generate,
        )
    ...
    def evaluate(
        ...
        def generate():
            ...
            current_progress = current_progress_before = DBSession.execute(
                select([t_progress.c.value]).where(
                    and_(
            ...
            if not current_progress:
                ...
                current_progress = next(
                    (
                        x["value"]
                        for x in Achievement.compute_progress(achievement, compared_subject, achievement_date)
            ...
            if is_player:
                goal = evaluate_value_expression(achievement["goal"], {"level": subject_wants_level})
            ...
            if current_progress != current_progress_before:
                if current_progress_before:
                    update_connection().execute(
                        t_progress.update({"value": current_progress}).where(
                            and_(
                                t_progress.c.subject_id == subject_id,
                                t_progress.c.achievement_id == achievement_id,
                                t_progress.c.achievement_date == AchievementDate.db_format(achievement_date),
            ...
            if is_player and execute_triggers:
                Achievement.select_and_execute_triggers(
                    ...
                    current_goal=goal,
                    previous_goal=evaluate_value_expression(achievement["goal"], {"level": subject_has_level}),
                    value=current_progress,
                    context_subject_id=context_subject_id,
            ...
            if generate_output is True and achievement["comparison_type"] in (
                "relations",
                "global",
                "context_subject",
            ):
            ...
            if achieved and subject_has_level < achievement["maxlevel"]:
                    ...
                    new_level_output = {
                        "rewards": {
                            str(r["id"]): {
                                ...
                                "value_translated": Translation.trs(
                                    r["value_translation_id"],
                                    {"level": subject_wants_level},
                                ),
                            }
                ...
                evaluation = (
                    update_connection()
                    .execute(
                        select([t_evaluations.c.id]).where(
                            and_(
                                t_evaluations.c.subject_id == subject_id,
                ...
                if evaluation:
                    update_connection().execute(
                        t_evaluations.update().values({"achieved": True}).where(t_evaluations.c.id == evaluation["id"])
                    )
                else:
                    update_connection().execute(
                        t_evaluations.insert().values(
                            {
                                "subject_id": subject_id,
                                "achievement_id": achievement["id"],
                                "achievement_date": AchievementDate.db_format(achievement_date),
                ...
                cache_achievements_subjects_levels.delete(
                    "%s_%s_%s_%s"
                    % (
                        str(subject_id),
                        str(achievement_id),
                ...
                output.update(
                    {
                        ...
                        "levels_achieved": {
                            str(x["level"]): x["achieved_at"]
                            for x in Achievement.get_level(
                                subject_id,
                                achievement["id"],
                                achievement_date,
                                context_subject_id,
            ...
            if (
                generate_output and new_level_output is not None
            ):  # if we reached a new level in this recursion step, add the previous levels rewards and properties
    ...
    def get_rewards(cls, achievement_id, level):
        ...
        this_level = DBSession.execute(
            select(
                [
                    t_rewards.c.id.label("reward_id"),
            ...
            .where(
                and_(
                    or_(
                        t_achievements_rewards.c.from_level <= level,
        ...
        prev_level = DBSession.execute(
            ...
            .where(
                and_(
                    or_(
                        t_achievements_rewards.c.from_level <= level - 1,
                        t_achievements_rewards.c.from_level == None,
                    ),
                    t_achievements_rewards.c.achievement_id == achievement_id,
                )
        ...
        build_hash = lambda x, l: hashlib.md5(
            (
                str(x["id"])
                + str(evaluate_string(x["value"], {"level": l}))
                + str(Translation.trs(x["value_translation_id"], {"level": l}))
            ).encode("UTF-8")
        ).hexdigest()
    ...
    def get_achievement_properties(cls, achievement_id, level):
        ...
        return DBSession.execute(
            select(
                [
                    t_achievementproperties.c.id.label("property_id"),
            ...
            .where(
                and_(
                    or_(
                        t_achievements_achievementproperties.c.from_level <= level,
    ...
    def compute_progress(cls, achievement, subject, achievement_date):
        ...
        def generate_statement_cache():
            ...
            # prepare
            select_cols = [
                func.sum(t_values.c.value).label("value"),
            ...
            if group_by_dateformat:
                ...
                if timezone:
                    datetime_col = func.to_char(
                        text("values.datetime AT TIME ZONE '%s'" % (timezone,)),
                        group_by_dateformat,
                    ).label("datetime")
                else:
                    datetime_col = func.to_char(
                        text("values.datetime AT TIME ZONE subjects.timezone"),
                        group_by_dateformat,
                    ).label("datetime")
            ...
            if datetime_col is not None or group_by_key is not False:
                ...
                combined_user_query = select(select_cols2, from_obj=query_with_groups).group_by(
                    query_with_groups.c.subject_id
                )
    ...
    @classmethod
    def select_and_execute_triggers(
        cls,
        achievement,
        achievement_date,
        subject,
        ...
        executions = {
            r["achievement_trigger_id"]: r["step"]
            for r in DBSession.execute(
                select(
                    [
                        t_achievement_trigger_steps.c.id.label("step_id"),
                    ...
                    and_(
                        t_achievement_triggers.c.achievement_id == achievement["id"],
                        t_achievement_trigger_step_executions.c.achievement_date
                        == AchievementDate.db_format(achievement_date),
                        t_achievement_trigger_step_executions.c.subject_id == subject_id,
                        t_achievement_trigger_step_executions.c.execution_level == level,
                    )
                )
            ).fetchall()
        }
        ...
        trigger_steps = DBSession.execute(
            select(
                [
        ...
        trigger_steps = [
            s for s in trigger_steps if s["step"] > executions.get(s["achievement_trigger_id"], -sys.maxsize)
        ]
            ...
            for step in trigger_steps:
                if step["condition_type"] == "percentage" and step["condition_percentage"]:
                    ...
                    if current_percentage >= 1.0 and required_percentage != 1.0 and not step["execute_when_complete"]:
                        ...
                        continue
                    if (operator == "geq" and current_percentage >= required_percentage) or (
                        operator == "leq" and current_percentage <= required_percentage
                    ):
                        if (
                            exec_queue.get(step["achievement_trigger_id"], {"step": -sys.maxsize})["step"]
                            < step["step"]
                        ):
    ...
    def get_leaderboard(cls, achievement, achievement_date, subject_ids, context_subject_id):
        ...
        q = (
            select([t_progress.c.subject_id, t_progress.c.value])
            .where(
                and_(
                    t_progress.c.subject_id.in_(subject_ids),
                    t_progress.c.achievement_id == achievement["id"],
                    t_progress.c.achievement_date == AchievementDate.db_format(achievement_date),
                )
            )
            .order_by(t_progress.c.value.desc(), t_progress.c.subject_id.desc())
        )
        ...
        positions = [
            {
                "subject": Subject.basic_output(subjects[items[i]["subject_id"]]),
                "value": items[i]["value"],
                "position": i,
            }
...
class AchievementReward(ABase):
    ...
    def get_achievement_reward(cls, achievement_reward_id):
        return DBSession.execute(
            t_achievements_rewards.select(t_achievements_rewards.c.id == achievement_reward_id)
        ).fetchone()
...
class Translation(ABase):
    ...
    def trs(cls, translation_id, params={}):
        ...
        try:
            ...
            ret = {
                str(x["name"]): evaluate_string(x["text"], params) for x in cls.get_translation_variable(translation_id)
            }
        except Exception as e:
            ret = {str(x["name"]): x["text"] for x in cls.get_translation_variable(translation_id)}
            log.exception(
                "Evaluation of string-forumlar failed: %s"
                % (ret.get(get_settings().get("fallback_language", "en"), translation_id),)
            )
    ...
    def get_translation_variable(cls, translation_id):
        return DBSession.execute(
            select(
                [t_translations.c.text, t_languages.c.name],
                from_obj=t_translationvariables.join(t_translations).join(t_languages),
            ).where(t_translationvariables.c.id == translation_id)
        ).fetchall()
...
class SubjectMessage(ABase):
    def __unicode__(self, *args, **kwargs):
        return "Message: %s" % (
            Translation.trs(self.translation_id, self.params).get(get_settings().get("fallback_language", "en")),
        )
    ...
    def deliver(cls, message):
        ...
        j = t_subjects.join(t_languages)
        subject_language = DBSession.execute(
            select([t_languages.c.name], from_obj=j).where(t_subjects.c.id == message["subject_id"])
        ).fetchone()
        if subject_language:
            language = subject_language["name"]
        ...
        if not message["has_been_pushed"]:
            try:
                ...
                DBSession.execute(
                    t_subject_messages.update()
                    .values({"has_been_pushed": True})
                    .where(t_subject_messages.c.id == message["id"])
                )
...
class AchievementTriggerStep(ABase):
    ...
    def execute(
        ...
        uS.execute(
            t_achievement_trigger_step_executions.insert().values(
                {
                    "subject_id": subject_id,
        ...
        properties = {r["name"]: Translation.trs(r["value_translation_id"], {"level": level}) for r in properties}
        ...
        if not suppress_actions:
            if trigger_step["action_type"] == "subject_message":
                ...
                action_value = evaluate_value_expression(trigger_step["action_value"], {"level": level})
                action_subject_type_id = trigger_step["action_subjecttype_id"]
                action_variable_id = trigger_step["action_variable_id"]
                action_variable = DBSession.execute(
                    select([t_variables.c.name], from_obj=t_variables).where(t_variables.c.id == action_variable_id)
                ).fetchone()
...
mapper(
    AuthToken,
    t_auth_tokens,
    properties={"user": relationship(AuthUser, backref="tokens")},
)
...
mapper(
    ...
    properties={
        ...
        "subsubjects": relationship(
            Subject,
            secondary=t_subjects_subjects,
...
mapper(
    ...
    properties={
        "subtypes": relationship(
            SubjectType,
            secondary=t_subjecttypes_subjecttypes,
...
mapper(
    ...
    properties={
        ...
        "player_subjecttype": relationship(
            SubjectType,
            primaryjoin=t_achievements.c.player_subjecttype_id == t_subjecttypes.c.id,
        ),
        "context_subjecttype": relationship(
            SubjectType,
            primaryjoin=t_achievements.c.context_subjecttype_id == t_subjecttypes.c.id,
        ),
        "compared_subjecttypes": relationship(
            SubjectType,
...
mapper(
    ...
    t_achievements_achievementproperties,
    properties={
        "property": relationship(AchievementProperty, backref="achievements"),
        "value_translation": relationship(TranslationVariable),
    },
)
mapper(Reward, t_rewards, properties={"rewarded_subjecttype": relationship(SubjectType)})
mapper(
    AchievementReward,
    t_achievements_rewards,
    properties={
...
mapper(
    AchievementTrigger,
    t_achievement_triggers,
    properties={
        "achievement": relationship(Achievement, backref="triggers"),
    },
)
mapper(
    AchievementTriggerStep,
    t_achievement_trigger_steps,
    properties={
