...
def add_or_update_subject(request):
    ...
    if asbool(get_settings().get("enable_user_authentication", False)):
        ...
        may_update = (
            request.has_perm(perm_global_manage_subjects)
            or request.has_perm(perm_own_update_subject_infos)
            and request.subject.id == subject_id
        )
        if not may_update:
...
def delete_subject(request):
    ...
    if asbool(get_settings().get("enable_user_authentication", False)):
        # ensure that the subject exists and we have the permission to update it
        may_delete = (
            request.has_perm(perm_global_delete_subject)
            or request.has_perm(perm_own_delete_subject)
            and request.subject.id == subject_id
        )
        if not may_delete:
...
def _get_progress(
                ...
                for context_subject_id in relevant_context_ids:
                    evaluatelist.append(
                        ea(
                            ...
                            achievement_date,
                            context_subject_id,
                            execute_triggers=(i == 0 or i == 1 or achievement_date == None),
                        )
                    )
                    i += 1
    ...
    ret = {
        ...
        "achievement_errors": [x for x in evaluatelist if x != None and "error" in x],
    }
...
def get_progress(request):
    ...
    output = _get_progress(
        achievements_for_subject=subject,
                ...
                if asbool(get_settings().get("enable_user_authentication", False)):
                    if not Variable.may_increase(variable, request, subject_id):
                        raise APIError(
                            403,
                            "forbidden",
                            "You may not increase the variable %s for subject %s." % (variable_name, subject_id),
                        )
...
def get_achievement_level(request):
    ...
    level_output = (
        Achievement.basic_output(achievement, True, level)
        .get("levels")
        .get(str(level), {"properties": {}, "rewards": {}})
    )
...
def auth_login(request):
    ...
    if user:
        ...
        if not email or not password:
            raise APIError(
                400,
                "login.email_and_password_required",
        ...
        if not user or not user.verify_password(password):
            raise APIError(
                401,
                "login.email_or_password_invalid",
...
def change_password(request):
    ...
    if not user or not user.verify_password(old_password):
        raise APIError(
            401,
            "change_password.email_or_old_password_invalid",
    ...
    if new_password == old_password:
        raise APIError(
            400,
            "change_password.may_not_be_the_same",
    ...
    if not AuthUser.check_password_strength(new_password):
        raise APIError(
            400,
            "change_password.invalid_new_password",
...
def register_device(request):
    ...
    if not device_id or not push_id or not subject_id or not device_os or not app_version:
        raise APIError(
            400,
            "register_device.required_fields",
            "Required fields: device_id, push_id, device_os, app_version",
        )

    if asbool(get_settings().get("enable_user_authentication", False)):
        may_register = (
            request.has_perm(perm_global_register_device)
            or request.has_perm(perm_own_register_device)
    ...
    if not exists_by_expr(t_subjects, t_subjects.c.id == subject_id):
        raise APIError(
            404,
            "register_device.subject_not_found",
...
def get_messages(request):
    ...
    if asbool(get_settings().get("enable_user_authentication", False)):
        may_read_messages = (
            request.has_perm(perm_global_read_messages)
            or request.has_perm(perm_own_read_messages)
            and str(request.subject.id) == str(subject_id)
        )
    ...
    if not exists_by_expr(t_subjects, t_subjects.c.id == subject_id):
        raise APIError(404, "get_messages.subject_not_found", "There is no subject with this id.")

    q = (
        t_subject_messages.select()
        .where(t_subject_messages.c.subject_id == subject_id)
        .order_by(t_subject_messages.c.created_at.desc())
        .limit(limit)
        .offset(offset)
    )
...
def set_messages_read(request):
    ...
    if not exists_by_expr(t_subjects, t_subjects.c.id == subject_id):
        raise APIError(
            404,
            "set_messages_read.subject_not_found",
            "There is no subject with this id.",
        )

    message_id = doc.get("message_id")
    q = select(
        [t_subject_messages.c.id, t_subject_messages.c.created_at],
        from_obj=t_subject_messages,
    ).where(
        and_(
            t_subject_messages.c.id == message_id,
    ...
    if not msg:
        raise APIError(
            404,
            "set_messages_read.message_not_found",
